-- Fix Database Tables Step by Step
...
-- Step 2: Create attendance_records table WITHOUT foreign key first, then add it
...
-- Step 5: Check if freelancers table has freelancer_id
-- If not, we need to check the structure first

-- Step 6: Create freelancer_sessions table (check freelancer_id exists first)
